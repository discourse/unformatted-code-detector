...
if (include_html) {
  _codeEnergyIndicators[
    CodeEnergyLevels.Complex
  ].indicators = _codeEnergyIndicators[
    CodeEnergyLevels.Complex
  ].indicators.concat(htmlIndicators);
}

export const codeEnergyIndicators = Object.entries(_codeEnergyIndicators)
  .map(([key, { indicators }]) => {
